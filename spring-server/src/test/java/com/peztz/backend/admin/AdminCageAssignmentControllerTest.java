...
class AdminCageAssignmentControllerTest {
	...
	@Test
	void facilityManagerCanRegisterListAndReadCameraRuntimeStatus() throws Exception {
		Facility facility = saveFacility("Facility Hospital");
		AppUser manager = saveUser("manager@example.com", "Manager", "HOSPITAL", facility.getId());
		saveToken("manager-token", manager);
		Cage cage = cageRepository.save(Cage.builder()
				.id(UUID.randomUUID())
				.facility(facility)
		...
		MvcResult cameraResult = mockMvc.perform(put("/api/facilities/{facilityId}/cages/{cageId}/camera",
						facility.getId(), cage.getId())
					.header("Authorization", "Bearer manager-token")
					.contentType(MediaType.APPLICATION_JSON)
					.content(objectMapper.writeValueAsString(Map.of("name", "Demo Camera"))))
				.andExpect(status().isOk())
				.andExpect(jsonPath("$.cageId").value(cage.getId().toString()))
				.andExpect(jsonPath("$.name").value("Demo Camera"))
				.andExpect(jsonPath("$.streamStatus").value("IDLE"))
				.andReturn();
		String cameraId = objectMapper.readTree(cameraResult.getResponse().getContentAsString())
				.get("cameraId").asText();

		mockMvc.perform(get("/api/facilities/{facilityId}/cameras", facility.getId())
						.header("Authorization", "Bearer manager-token"))
				.andExpect(status().isOk())
				.andExpect(jsonPath("$[0].cameraId").value(cameraId))
				.andExpect(jsonPath("$[0].cageId").value(cage.getId().toString()))
				.andExpect(jsonPath("$[0].name").value("Demo Camera"));

		mockMvc.perform(get("/api/facilities/{facilityId}/cameras/{cameraId}/runtime-status",
						facility.getId(), cameraId)
					.header("Authorization", "Bearer manager-token"))
				.andExpect(status().isOk())
				.andExpect(jsonPath("$.cameraId").value(cameraId))
