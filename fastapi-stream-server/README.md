...
- `POST /internal/reports/daily/generate`: Spring이 전달한 익명화 로그를 OpenAI로 구조화
...
OPENAI_API_KEY                  # 일일 리포트 생성용 OpenAI 프로젝트 API 키
...
일일 리포트 API는 PostgreSQL에 직접 접속하지 않습니다. 보호자 권한 확인, 날짜별 로그
조회와 결과 저장은 Spring이 담당하며 FastAPI에는 이름·품종과 분석에 필요한 로그 필드만
전달합니다. OpenAI 응답은 Pydantic Structured Outputs로 검증한 카드형 JSON입니다.
